...
export function Log(type:LogType, message:any, _debug?:boolean, _options?:logOpts): void {
    ...
    if(options && options.file && !options.forward) {
        ...
        let stream = LogStreamCollection[options.file]
        if(typeof stream !== typeof WriteStream) {
            stream = createWriteStream(options.file);
        }

        if(typeof stream === "undefined") {
            throw new Error(`Logger error: Could not create write stream: ${options.file}`);
        }
